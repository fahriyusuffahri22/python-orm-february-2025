...
def product_quantity_ordered():
    return '\n'.join(
        f'Quantity ordered of {entry["name"]}: {entry["total_ordered_quantity"]}'
        for entry in Product.objects
        .values('name')
        .annotate(total_ordered_quantity=Sum('orderproduct__quantity'))
        .exclude(total_ordered_quantity=0)
...
def ordered_products_per_customer():
    ...
    for order in Order.objects.all().select_related('customer').prefetch_related('products__category').order_by('id'):
...
def filter_products():
    return '\n'.join(
        ...
        .values('name', 'price')
        .filter(is_available=True, price__gt=3)
        .order_by('-price', 'name')
    )


def give_discount():
    Product.objects.filter(is_available=True, price__gt=3).update(price=F('price') * 0.7)
