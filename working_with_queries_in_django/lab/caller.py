...
def find_authors_nationalities():
    return '\n'.join(
        ...
        for author in Author.objects.exclude(nationality=None).values('first_name', 'last_name', 'nationality')
    )
...
def order_books_by_year():
    return '\n'.join(
        ...
        .values('publication_year', 'title', 'author')
    )


def delete_review_by_id(_id):
...
def filter_authors_by_nationalities(nationality):
    return '\n'.join(
        f'{author["biography"]}' if author["biography"] else f'{author["first_name"]} {author["last_name"]}'
        for author in Author.objects
        .filter(nationality=nationality)
        .order_by('first_name', 'last_name')
        .values('biography', 'first_name', 'last_name')
    )
...
def filter_authors_by_birth_year(start_year, end_year):
    return '\n'.join(
        ...
        .order_by('-birth_date')
        .values('birth_date', 'first_name', 'last_name')
    )
